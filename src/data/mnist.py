...
if __name__ == '__main__':
    data = MNISTDataset(n_val=2000, reduced_test_set=False)
    print(data.X[data.test_indices])
    print(len(data.X[data.test_indices]))
    """"
    np.set_printoptions(linewidth=200)
    idx = 0
    import re
    string = str((data.X[idx]/255).round(2)).replace('[', '{').replace(']', '}')
    #print(string)
