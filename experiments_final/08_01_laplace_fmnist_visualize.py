...
title = 'Fashion MNIST - Last layer Laplace approximation (w. full Hessian)'
def plot_al_curve(
    data,
    metric='Accuracy',
    aggregates=['mean', 'std'],
...
def main():
    ...
    for file in files:
        ...
        dfs.append(
            pd.DataFrame({
                ...
                'la_acc_test': tracker.get_stat('la_acc_test'),
                ...
                'la_ce_test': tracker.get_stat('la_ce_test'),
                'la_avg_conf_test': tracker.get_stat('la_avg_conf_test'),
                'la_avg_entropy_test': tracker.get_stat('la_avg_entropy_test'),
                'la_avg_bald_rt_test': tracker.get_stat('la_avg_bald_rt_test'),
                'la_avg_bald_test': tracker.get_stat('la_avg_bald_test'),
    ...
    fig, ax = plt.subplots(figsize=(8,6))
    line_styles = ['--', ':', '-']
    metrics = ['la_avg_entropy_test', 'la_avg_bald_rt_test', 'la_avg_bald_test']
    ...
    for i, af in enumerate(sorted(data['acquisition_function'].unique())):
        ...
        for j, metric in enumerate(metrics):
            if af == 'max_entropy' and metric != 'la_avg_entropy_test':
                continue
            if af == 'max_entropy':
                line_style = '-'
            else:
