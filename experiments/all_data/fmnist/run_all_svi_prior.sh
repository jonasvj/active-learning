...
base_command="\
    ...
    cd ~/active-learning;\
    python3 ${script_dir}/train_fmnist.py"
...
# Other params
wd=0
sigma=0.2041241452319315 # (1/sqrt(N))*(1/sqrt(wd)) # N=48000 (50000 train - 2000 val), wd=5e-4

# Build commands
command_list=()
bsub_args_list=()

methods=("swag_svi") #("deterministic" "ensemble" "laplace" "laplace_ensemble" "swag" "swag_svi")
for seed in "${seeds[@]}"; do
    for method in "${methods[@]}"; do
        ...
        full_command="${base_command} seed=${seed} inference_key=${method} data_path=${data_path} \
            model.model_hparams.sigma_b=${sigma} model.model_hparams.sigma_w=${sigma} \
            model.model_hparams.sigma_default=${sigma} model.model_hparams.use_prior=true \
            fit_model_hparams.weight_decay=0 swag_svi.fit_hparams.fit_swag_hparams.swag_lr=1e-3"
...
for seed in "${seeds[@]}"; do
    for nt in "${n_transforms[@]}"; do
        ...
        full_command="${base_command} seed=${seed} inference_key=${method} data_path=${data_path} \
            ...
            model.model_hparams.sigma_default=${sigma} model.model_hparams.use_prior=true \
            fit_model_hparams.weight_decay=0 swag_svi.fit_hparams.fit_swag_hparams.swag_lr=1e-3 \
            laplace_nf.fit_hparams.fit_flow_hparams.n_transforms=${nt}"
        command_list+=("${full_command}")
