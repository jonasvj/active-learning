...
data.data_hparams.reduced_test_set: true

methods=("laplace" "laplace_ensemble") #("deterministic" "ensemble" "laplace" "laplace_ensemble" "swag" "swag_svi")
...
for seed in "${seeds[@]}"; do
    for nt in "${n_transforms[@]}"; do
        ...
        full_command="${base_command} seed=${seed} inference_key=${method} data_path=${data_path} \
            laplace_nf.fit_hparams.fit_flow_hparams.n_transforms=${nt} data.data_hparams.reduced_test_set=true"
        command_list+=("${full_command}")
