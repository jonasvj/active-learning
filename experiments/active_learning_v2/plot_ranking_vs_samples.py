...
def plot_stats(fig, ax, dfs, keys, x_metric, y_metric, **kwargs):
    n = 0
    ranks = list()
    for key in keys:
        df = dfs[key]
        df = df[['seed', 'method', 'n_transforms', y_metric]]
        
        for name, df_group in df.groupby(['seed']):
            array = np.array(df_group[y_metric].tolist())
            new_ranks = rankdata(-array, axis=0, nan_policy='omit')
            
            ranks.append(new_ranks)
    
    ranks = np.array(ranks)
    y_mean = np.nanmean(ranks,axis=0)
  
    for i, (name, df_group) in enumerate(dfs[keys[0]].groupby(['method', 'n_transforms'])):
        print(i, name)
...
def main():
    ...
    for j, aq_func in enumerate(aq_funcs):
        keys = [dataset + '_' + aq_func for dataset in datasets]
        plot_stats(
            fig,
            ax[i,j],
            dfs,
    ...
    plt.tight_layout()
    savefig(
        'experiments/active_learning_v2/ranking_vs_samples', 
        bbox_inches='tight'
    )
