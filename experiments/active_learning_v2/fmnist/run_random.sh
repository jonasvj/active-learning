...
base_command="\
    ...
    cd ~/active-learning;\
    python3 ${script_dir}/active_learning.py"
...
methods=("deterministic" "mcdo" "ensemble" "laplace" "laplace_ensemble" "swag" "swag_svi")
for seed in "${seeds[@]}"; do
    for method in "${methods[@]}"; do
        ...
        full_command="${base_command} seed=${seed} inference_key=${method} data_path=${data_path} \
            al.acquisition_function=random_acquisition"
...
for seed in "${seeds[@]}"; do
    for nt in "${n_transforms[@]}"; do
        ...
        full_command="${base_command} seed=${seed} inference_key=${method} data_path=${data_path} \
            laplace_nf.fit_hparams.fit_flow_hparams.n_transforms=${nt} al.acquisition_function=random_acquisition"
        command_list+=("${full_command}")
