...
def plot_stats(
    fig, ax, df, x_metric, y_metric, x_std=None, y_std=None, ma=True, **kwargs
):
    ...
    for name, df_group in df.groupby(['method', 'n_transforms']):
        ...
        #alphas = np.linspace(0.5, 1, len(x_mean))
        #ax.scatter(x_mean, y_mean, alpha=alphas, c=color_dict[disp_name], edgecolors='none', s=5)
        ax.scatter(x_mean[0], y_mean[0], c=color_dict[disp_name], edgecolors='none', s=5)
        ax.errorbar(
            x=x_mean,
            xerr=x_std,
            y=y_mean,
...
def main():
    ...
    savefig(
        'experiments/active_learning_v2/llh_vs_ece', 
        bbox_inches='tight'
    )
    ...
    plt.tight_layout(h_pad=5/3)
    savefig(
        'experiments/active_learning_v2/llh_vs_ece_raw', 
        bbox_inches='tight'
    )
