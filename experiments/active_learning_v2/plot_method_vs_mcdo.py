...
def plot_stats(
    ...
    for name, df_group in df.groupby(['method', 'n_transforms']):
        ...
        if disp_name == 'MCDO':
            mcdo_array = np.array(df_group[y_metric].tolist())
            mcdo_mean = np.nanmean(mcdo_array, axis=0)
    ...
    for name, df_group in df.groupby(['method', 'n_transforms']):
        ...
        y_mean = y_mean - mcdo_mean

        disp_name = display_name_dict[name[0]]
        if disp_name == 'LA-NF-':
            disp_name += f'{name[1]}'
        
        if disp_name in ['MCDO', 'SWAG-SVI']:
            continue
...
def main():
    ...
    for i, dataset in enumerate(datasets):
        for j, aq_func in enumerate(aq_funcs):
            ...
            if j == 0:
                ax[i,j].set_ylabel(r'$\mathrm{LLH} - \mathrm{LLH}_{\mathrm{MCDO}}\uparrow$')
    ...
    savefig(
        'experiments/active_learning_v2/method_vs_mcdo', 
        bbox_inches='tight'
    )
