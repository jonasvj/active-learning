...
# Folder for plots
plot_folder="${run_folder%run}plots"
mkdir -p ${plot_folder}
...
model_params="\
    ...
    model.model_hparams.drop_probs=[0.00] \
...
inference_params="\
    inference=nf_refined_ll_laplace \
    inference.inference_class=NFRefinedLastLayerLaplace \
    ...
    inference.fit_hparams.fit_model_hparams.early_stopping_patience=5000 \
    inference.fit_hparams.fit_model_hparams.min_epochs=1 \

    inference.fit_hparams.fit_laplace_hparams.hessian_structure=full \
    
    inference.fit_hparams.fit_flow_hparams.n_epochs=20000 \
    inference.fit_hparams.fit_flow_hparams.lr=1e-3 \
    inference.fit_hparams.fit_flow_hparams.transform=Radial \
    inference.fit_hparams.fit_flow_hparams.n_transforms=30 \
    inference.fit_hparams.fit_flow_hparams.num_particles=32"

# Figure path
fig_path="fig_path=${plot_folder}/${experiment_name}_20"
