...
#set_matplotlib_defaults(font_size=6, font_scale=7)
...
def plot_conf(data, confidence):
    ...
    ax.set_title(r'$\mathrm{Confidence}\left[f_{\boldsymbol{\theta}}(\boldsymbol{x})\right]$', fontsize=8*3, pad=8)
    return fig, ax
...
def plot_pca(
    ...
    ax.set_aspect('equal')
    ax.set_xlabel(fr'PC{components[0]+1}', fontsize=8*3, fontfamily='serif')
    ax.set_ylabel(fr'PC{components[1]+1}', fontsize=8*3, fontfamily='serif')
    ax.set_title(r'$-\log p(\mathcal{D},\boldsymbol{\theta})$', fontsize=8*3, pad=8)
   ...
   # Load data
    ...
    for file in files:
        ...
        if 'samples' in experiment_data:
            ...
            fig, ax = plot_conf(data, confidence)
            plt.tight_layout()
            savefig(fig_path)#, bbox_inches='tight')
            plt.close()
